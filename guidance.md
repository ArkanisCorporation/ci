...
| `wf-setup-node.yml`           | TS/JS lint/test/build              | `node-version`, `package-manager`, `workspace`, `script-*`, `runs-on-json`                              | test reports, coverage, bundle stats                              | use `setup-node` package-manager cache; lockfile strict installs. ([GitHub][11])                                         |
